...
from social_network.user_profile.models import FriendList, Profile
...
class ChatsListViewTestCase(TestCase):
    def setUp(self):
        ...
        self.message1 = Message.objects.create(
            author=self.profile1, receiver=self.profile2
        )
        self.message2 = Message.objects.create(
            author=self.profile2, receiver=self.profile1
        )
...
class MessageListViewTestCase(TestCase):
    def setUp(self):
        ...
        self.profile2 = Profile.objects.get(user=self.user2)
        self.message1 = Message.objects.create(
            author=self.profile1, receiver=self.profile2, message="hello"
        )
        self.message2 = Message.objects.create(
            author=self.profile2, receiver=self.profile1, message="hi"
        )
    ...
    def test_view_url_accessible_by_name(self):
        ...
        response = self.client.get(
            reverse("messages", kwargs={"friend_id": self.profile2.id})
        )
        self.assertEqual(response.status_code, 200)
...
class MessageCreateViewTest(TestCase):
    def setUp(self):
        ...
        self.profile2 = Profile.objects.get(user=self.user2)
        self.message_data = {"message": "Hello, World!"}

    def test_create_message(self):
        self.client.login(email="testuser1@email.com", password="Testpass123")
        response = self.client.post(
            reverse("message_create", kwargs={"receiver_id": self.profile2.id}),
