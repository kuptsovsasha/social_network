...
class ChatsListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        friends_chats_ids = list(
            Message.objects.filter(author=profile).values_list("receiver_id", flat=True)
        )
        if friends_chats_ids:
            # if user already write to someone - add ids profile who write to user
            friends_chats_ids.extend(
                list(
                    Message.objects.filter(receiver=profile).values_list(
                        "author_id", flat=True
                    )
                )
            )
        ...
        try:
            friends = Profile.objects.filter(id__in=friends_chats_ids).order_by(
                "user__username"
            )
            return friends
        except TypeError:
...
class MessageListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        messages = Message.objects.filter(
            Q(author=profile, receiver=friend) | Q(author=friend, receiver=profile)
        ).order_by("created_at")
        return messages
