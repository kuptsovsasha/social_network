...
from django.views.generic import ListView, TemplateView, DetailView, RedirectView, View
...
class FindProfilesView(LoginRequiredMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        friends = requested_user_profile.friends.all().values_list('profile__user__email', flat=True)
        friends_request = FriendRequest.objects.filter(sender=requested_user_profile, is_active=True).\
            values_list('receiver__user__email', flat=True)

        html_results = render_to_string('search_results_partial.html',
                                        {'results': profiles, 'friends': friends, 'friends_requests': friends_request})
...
class ProfileFriendsListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        try:
            return profile.friend_list_profile.friends.exclude(user=self.request.user).order_by('user__username')
